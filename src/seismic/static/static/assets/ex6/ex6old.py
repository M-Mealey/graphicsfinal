...
# plot_model()
printModel("-JSON", "-file", "Example6.json")

#recorder('Node','-file','Node.out','-time','-node',1,2,3,4,5,'-dof',1,'reaction')
...
recorder('Node','-file','Node.out','-time','-dof',1,2,3,'disp')
recorder("Element", "-file", "ElemShell.out", "-time", "-eleRange", 0, 40, "forces")
recorder("Element", "-file", "ElemShelltest.out", "-time", "-ele", 6, "forces")
recorder('Element', '-file', "ElemBeam.out", "-time", '-closeOnWrite', "-eleRange", 41, 80, 'forces')
recorder('Element', '-file', "ElemBeamtest.out", "-time", '-closeOnWrite', "-ele", 41, 'forces')
recorder('Element', '-file', "ElemBeamlocaltest.out", "-time", '-closeOnWrite', "-ele", 41, 'localForce')
...
if(AnalysisType=="Pushover"):
	...
	algorithm('KrylovNewton')
	test('NormDispIncr',1e-05, 1000, 0)
	analysis("Static")
	...
	#recorder('Node', '-file', "PushoverOut/React.out", '-closeOnWrite','-time', '-node', 1, 2, 3, 4, 5, '-dof',1, 'reaction')
	#recorder('Node', '-file', "PushoverOut/Disp.out", '-closeOnWrite','-time', '-node', ControlNode, '-dof',1, 'disp')
...
with open("struct.ts", "w+") as f:
    with open("Example6.json","r+") as src:
      f.write("export const ex6data = `\n"+src.read()+"\n`\n")

with open("nodedata.ts", "w+") as f:
    with open("Node.out","r+") as src:
      f.write("export const ex6node = `\n"+src.read()+"\n`\n")

with open("elemdata.ts", "w+") as f:
    with open("ElemShell.out","r+") as src:
      ...
      for line in data:
        ...
        while ind<len(info):
            for x in range(6):
                avg = (float(info[ind+x])+float(info[ind+x+6])+float(info[ind+x+12])+float(info[ind+x+18])) /4.0
                new_line.append(str(avg))
            ind += 24
        new_line = " ".join(new_line)
        output.append(new_line)
      output = "\n".join(output)
      f.write("export const ex6elem = `\n"+output+"\n`\n")
    f.write("\n\n")
    with open("ElemBeam.out","r+") as src:
      data = src.read().splitlines()
      output = []
      for line in data:
        info = line.split(" ")
        ...
        while ind<len(info):
            for x in range(3):
                new_line.append(info[ind+x])
            ind += 6
        new_line = " ".join(new_line)
        output.append(new_line)
      output = "\n".join(output)
      f.write("export const ex6beam = `\n"+output+"\n`\n")