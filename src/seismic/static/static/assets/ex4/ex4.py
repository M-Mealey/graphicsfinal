...
#printModel("-JSON", "-file", "Example4.json")
# ------------------------------
...
if(AnalysisType=="Pushover"):
	...
	#recorder('Node', '-file', "Node.out", '-closeOnWrite', '-node', 2, '-dof',1, 'reaction')
	recorder('Node', '-file', "Node.out", "-time", '-closeOnWrite',  '-dof',1,2,3, 'disp')
	recorder('Element', '-file', "Elem.out", "-time", '-closeOnWrite', '-ele',1,2,3,11,12,13,21,22,23,101, 102,201,202,301,302, 'forces')
...
with open("struct.ts", "w+") as f:
    with open("Example4.json","r+") as src:
      f.write("export const ex4data = `\n"+src.read()+"\n`\n")

with open("nodedata.ts", "w+") as f:
    with open("Node.out") as src:
      f.write("export const ex4node = `\n"+src.read()+"\n`\n")

with open("elemdata.ts", "w+") as f:
    with open("Elem.out","r+") as src:
      f.write("export const ex4beam = `\n"+src.read()+"\n`\n")
